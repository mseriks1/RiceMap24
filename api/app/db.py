...
def _pg_translate_sql(sql: str) -> str:
    ...
    # SQLite accepts datetime('now') and datetime('now', '+90 days');
    # PostgreSQL does not. Translate the variants that still exist in older
    # code paths so auth/admin recovery does not crash on Render/PostgreSQL.
    out = re.sub(r"datetime\('now'\s*,\s*'\+([0-9]+)\s+days'\)", r"(CURRENT_TIMESTAMP + INTERVAL '\1 days')", out, flags=re.I)
    out = re.sub(r"datetime\('now'\s*,\s*'-([0-9]+)\s+days'\)", r"(CURRENT_TIMESTAMP - INTERVAL '\1 days')", out, flags=re.I)
    out = re.sub(r"datetime\('now'\s*,\s*'\+([0-9]+)\s+minutes'\)", r"(CURRENT_TIMESTAMP + INTERVAL '\1 minutes')", out, flags=re.I)
    out = re.sub(r"datetime\('now'\s*,\s*'-([0-9]+)\s+minutes'\)", r"(CURRENT_TIMESTAMP - INTERVAL '\1 minutes')", out, flags=re.I)
...
def _reset_admin_app_users_postgres_direct(email: str, password: str, display_name: str = "Admin") -> Dict[str, Any]:
    """PostgreSQL-native admin recovery path used on Render staging.

    This bypasses the sqlite-compat wrapper during emergency admin reset. Older
    staging databases may have partial auth tables or column defaults created by
    earlier steps; using explicit PostgreSQL DDL/DML here avoids wrapper/migration
    edge cases and only touches app_users/app_sessions.
    """
    ...
    with psycopg.connect(DB_URL, row_factory=dict_row) as conn:
        with conn.cursor() as cur:
            ...
            """)
            # Add any missing columns from old staging tables. Each statement is
            # isolated so one existing column cannot abort the whole reset.
            alters = [
                "ALTER TABLE app_users ADD COLUMN IF NOT EXISTS password_hash TEXT",
                "ALTER TABLE app_users ADD COLUMN IF NOT EXISTS display_name TEXT",
                "ALTER TABLE app_users ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'owner'",
                "ALTER TABLE app_users ADD COLUMN IF NOT EXISTS listing_id INTEGER",
                "ALTER TABLE app_users ADD COLUMN IF NOT EXISTS active INTEGER DEFAULT 1",
                "ALTER TABLE app_users ADD COLUMN IF NOT EXISTS email_verified INTEGER DEFAULT 0",
                "ALTER TABLE app_users ADD COLUMN IF NOT EXISTS last_login_at TEXT",
                "ALTER TABLE app_users ADD COLUMN IF NOT EXISTS created_at TEXT DEFAULT CURRENT_TIMESTAMP",
                "ALTER TABLE app_users ADD COLUMN IF NOT EXISTS updated_at TEXT DEFAULT CURRENT_TIMESTAMP",
                "ALTER TABLE app_sessions ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'owner'",
                "ALTER TABLE app_sessions ADD COLUMN IF NOT EXISTS revoked_at TEXT",
                "ALTER TABLE app_sessions ADD COLUMN IF NOT EXISTS updated_at TEXT DEFAULT CURRENT_TIMESTAMP",
            ]
            for stmt in alters:
                cur.execute(stmt)
            cur.execute("SELECT id FROM app_users WHERE lower(email)=lower(%s)", (email_norm,))
            row = cur.fetchone()
            if row:
                admin_id = int(row["id"])
                cur.execute("""
                    UPDATE app_users
                       SET password_hash=%s, display_name=%s, role='admin', listing_id=NULL,
                           active=1, email_verified=1, updated_at=CURRENT_TIMESTAMP
                     WHERE id=%s
                """, (pw_hash, (display_name or "Admin")[:120], admin_id))
            else:
                cur.execute("""
                    INSERT INTO app_users (email, password_hash, display_name, role, listing_id, active, email_verified, updated_at)
                    VALUES (%s, %s, %s, 'admin', NULL, 1, 1, CURRENT_TIMESTAMP)
                    RETURNING id
                """, (email_norm, pw_hash, (display_name or "Admin")[:120]))
                admin_id = int(cur.fetchone()["id"])
            cur.execute("UPDATE app_sessions SET revoked_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP WHERE role='admin' AND revoked_at IS NULL")
            cur.execute("UPDATE app_users SET active=0, updated_at=CURRENT_TIMESTAMP WHERE role='admin' AND id<>%s", (admin_id,))
            cur.execute("SELECT * FROM app_users WHERE id=%s", (admin_id,))
            user_row = cur.fetchone()
        conn.commit()
    return _user_row_to_dict(PgRow(user_row)) or {}
...
def reset_admin_app_users(email: str, password: str, display_name: str = "Admin") -> Dict[str, Any]:
    ...
    """
    if DB_ENGINE == "postgresql" and DB_URL:
        return _reset_admin_app_users_postgres_direct(email, password, display_name=display_name)
